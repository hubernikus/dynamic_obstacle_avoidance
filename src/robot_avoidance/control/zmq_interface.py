...
class ZMQInterface(object):
    ...
    def add_subscriber(self, subscriber_uri):
        ...
        assert isinstance(
            subscriber_uri, str
        ), "[ZMQInterface::add_subscriber] Argument 'subscriber_uri' of wrong type."
    ...
    def add_publisher(self, publisher_uri):
        ...
        assert isinstance(
            publisher_uri, str
        ), "[ZMQInterface::add_publisher] Argument 'publisher_uri' of wrong type."
        if publisher_uri not in self.publishers.keys():
            ...
            raise ValueError(
                "[ZMQInterface::add_publisher] There has already been a publisher registered to this URI!"
            )
    ...
    def send(self, publisher_uri, message):
        ...
        assert isinstance(
            publisher_uri, str
        ), "[ZMQInterface::send] Argument 'publisher_uri' of wrong type."
        assert isinstance(
            message, bytes
        ), "[ZMQInterface::send] Argument 'message' of wrong type."
        if publisher_uri in self.publishers.keys():
            res = self.publishers[publisher_uri].send(message, flags=0)
            return res is None
        else:
            raise ValueError(
                "[ZMQInterface::send] There has not been a publisher registered at this URI!"
            )
    ...
    def receive(self, subscriber_uri, flags=0):
        ...
        assert isinstance(
            subscriber_uri, str
        ), "[ZMQInterface::receive] Argument 'subscriber_uri' of wrong type."
        assert isinstance(
            flags, int
        ), "[ZMQInterface::receive] Argument 'flags' of wrong type."
        if subscriber_uri in self.subscribers.keys():
            ...
            raise ValueError(
                "[ZMQInterface::receive] There has not been a subscriber registered at this URI!"
            )
    ...
    def poll(self, subscriber_uri):
        ...
        assert isinstance(
            subscriber_uri, str
        ), "[ZMQInterface::poll] Argument 'subscriber_uri' of wrong type."
