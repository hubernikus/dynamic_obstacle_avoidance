...
class RobotAnimation():
    ...
    def start_animator(
        self, my_robot, initial_dynamics, obstacle_environment,
        ...
        # for ii in range(it_max):
            ...
            plt.plot(position_list[0, :ii+1], position_list[1, :ii+1], '.', color='k')
            for jj in range(my_robot.n_joints):
...
def simple_2link_robot(evaluate_jacobian=False):
    ...
    multi_robot_picture(
        ...
        x_lim=[-1.5, 2], y_lim=[-0.5, 2.5],
        delta_time=0.05,
        save_figure=True, it_draw_list=[1, 25, 60, 150])
