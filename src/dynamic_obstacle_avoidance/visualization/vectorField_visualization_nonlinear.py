...
def VectorFields_nonlinear(x_range=[0,10],y_range=[0,10], point_grid=10, obs=[], sysDyn_init=False, xAttractor = np.array(([0,0])), saveFigure = False, figName='default', noTicks=True, showLabel=True, figureSize=(7.,6), obs_avoidance_func=obs_avoidance_nonlinear_hirarchy, attractingRegion=False, drawVelArrow=False, colorCode=False, streamColor=[0.05,0.05,0.7], obstacleColor=[], plotObstacle=True, plotStream=True, figHandle=[], alphaVal=1, dynamicalSystem=linearAttractor, nonlinear=True, hirarchy=True, displacement_visualisation=True):
    ...
    for hh in np.arange(max_hirarchy+1)[ind_hirarchy]:
        if len(figHandle): 
            ...
            fig_ifd.canvas.set_window_title('Figure - Modulation at level {}'.format(hh))
        ...
        if plotStream:
            if colorCode:
                ...
                strm = res_ifd = ax_ifd.streamplot(XX, YY,dx1_noColl, dx2_noColl, color=velMag, cmaph='winter', norm=matplotlib.colors.Normalize(vmin=0, vmax=10.) )
        ...
        if plotObstacle:
            ...
            for n in range(len(obs)):
                ...
                obs_polygon_sf.append( plt.Polygon(obs[n].x_obs_sf, zorder=-2, alpha=0.2))
