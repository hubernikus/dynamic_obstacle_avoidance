...
class DynamicAnimationQOLO():
    def __init__(self, position_init, attractor_position=[0, 0], x_lim=None, y_lim=None, obstacle_list=None, ):
    ...
    def run_dynamic_animation(self, dt_simulation=0.01, max_it=500, scale_qolo=1, des_speed=0.3, dt_sleep=0.01, show_plot_vectorfield=False):
        ...
        self.agent = ObjectQOLO(center_position=self.position_init,
        ...
        while(True):
            ...
            self.agent.iterate_pos(obstacle_list=self.obstacle_list,
                                   des_speed=des_speed, dt=dt_simulation)
...
class ObjectQOLO(Obstacle):
    ...
    def __init__(self, attractor_position=None, ax=None, scale_qolo=1, **kwargs):
        if sys.version_info > (3, 0):
            ...
            super(CircularObstacle, self).__init__(
                name="QOLO", orientation=0, **kwargs
                )
        ...
        self.length_y = (1.0)*self.arr_img.shape[0]/self.arr_img.shape[1] * self.length_x 
    ...
    def iterate_pos(self, obstacle_list, ds_initial=None, attractor=None, dt=0.1, des_speed=0.3):
        ...
        ds_modulated = obs_avoidance_interpolation_moving(
                self.position, ds_initial, obstacle_list, 
        ...
        ds_modulated = make_velocity_constant(
            ds_modulated, self.position,
            self.attractor_position, constant_velocity=des_speed)
    ...
    def display_agent(self, rotation=None, ax=None, display_velocity=True, point_only=False):
        ''' Plot picture d (or only a point represetation) of the agent and it's velocity. '''
        
        if ax is not None:
            self.ax = ax

        self.ax.quiver(self.position[0], self.position[1], 
                       self.linear_velocity[0], self.linear_velocity[1],
                       color='b', alpha=0.7, scale=5.0,
                       label="Mouldated DS")
        ...
        if point_only:
            ...
            self.ax.imshow(
                arr_img_rotated,
                extent=[self.position[0]-lenght_x_rotated/2.0,
...
class LineObject(CircularObstacle):
    ...
    def __init__(self, people_path, ax):
        ...
        if sys.version_info>(3,0):
            super().__init__(radius=self.radius, center_position=self.position,
                             orientation=self.orientation,
                             margin_absolut=self.margin_absolut)
        else:
            super(CircularObstacle, self).__init__(radius=self.radius,
    ...
    def update_velocity_orientation(self, current_frame):
        ...
        # 2D-Velocity
        self.linear_velocity = (self.get_position(it=self.frame_list_it)-self.position) / delta_t
