...
def get_desired_radius(
    ...
    if obstacle_list.get_parent(it_obs) >= 0:
        intersection_points.append(obstacle_list.get_parent_intersection(it_child=it_obs))
    ...
    for it_child in obstacle_list.get_children(it_obs):
        intersection_points.append(obstacle_list.get_parent_intersection(it_child=it_child))
        # intersection_points.append(obstacle_list.get_convergence_boundary_point(
    ...
    boundary_projection = obstacle_list[it_obs].get_intersection_with_surface(
            direction=(position-obstacle_list[it_obs].center_position), in_global_frame=True)
    ...
    for ii in range(len(intersection_points)):
        for oo in range(len(obstacle_list)):
            ...
            gamma = obstacle_list[oo].get_gamma(boundary_projection, in_global_frame=True)
            if gamma <= 1:
                continue
            
            center = obstacle_list[it_obs].center_position
            dotprod = 1 - np.dot(position-center, intersection_points[ii]-center)/(
                np.linalg.norm(position-center)*np.linalg.norm(intersection_points[ii]-center))
...
def multihull_attraction(
    ...
    ind_obs = np.logical_and(gamma_array < cutoff_gamma_high, gamma_array > cutoff_gamma_low)
    if not any(ind_obs):
        return initial_velocity
    ...
    for it, oo in enumerate(np.arange(n_obstacles)[ind_obs]):
        ...
        convergence_radius = get_desired_radius(position, gamma_array[it], 
                                                it_obs=oo, obstacle_list=obstacle_list)
        # For testing:
