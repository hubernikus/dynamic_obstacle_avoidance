...
def obs_avoidance_potential_field(position, velocity, obs=[], xd=None,
    ...
    if evaluate_with_relative_minimum:
        limit_distance_repulsion = np.array([oo.get_maximal_distance() for oo in obs]).T * limit_distance_repulsion
    repulsive_force = constant_gain_repulsion * (1.0/dist_to_obstacles - 1.0/limit_distance_repulsion)
...
def obs_avoidance_orthogonal_moving(position, xd, obs=[], attractor='none', weightPow=2, repulsive_gammaMargin=0.01, repulsive_obstacle=False, velocicity_max=None, evaluate_in_global_frame=True, zero_vel_inside=False, cut_off_gamma=1e6, x=None, tangent_eigenvalue_isometric=True, gamma_distance=None):
    ...
    for n in np.arange(N_obs)[ind_obs]:
        ...
        temp, E_orth[:, :, n] = compute_decomposition_matrix(obs[n], pos_relative[:, n], in_global_frame=evaluate_in_global_frame)
