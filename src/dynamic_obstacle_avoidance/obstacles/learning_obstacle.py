...
def get_obstacle_from_scan(sensor_data, center_dist_scaling=1.5,
                           ...
                           input_is_polar=True, regression_obstacle=True,
                           center_position_offset=1.1, cutoff_distance=6,
                           print_timing=False):
    ...
    db_cluster = DBSCAN(eps=0.2, min_samples=5, metric="euclidean").fit(points_cartesian)
    ...
    for ii in range(n_clusters):
        ...
        if input_is_polar:
            ...
            center_position = transform_polar2cartesian(magnitude=ref_mag, angle=mean_angle)
        else:
        ...
        if regression_obstacle:
            obs_list.append(
                RegressionObstacle(center_position=center_position,
                                   surface_points=points_cartesian[ind_ii, :]))
        else:
            obs_list.append(
                GaussianEllipseObstacle(center_position=center_position,
                                        surface_points=points_cartesian[:, ind_ii]))
...
class RegressionObstacle(ObstacleFromData):
    ...
    def extend_with_boundary(self, boundary_values, angle_margin = 0.03):
        mag_surf, ang_surf = self.transform_cartesian2polar(self.surface_points, center_position=self.center_position)

        mag_boundary, ang_boundary = self.transform_cartesian2polar(boundary_values, center_position=self.center_position)

        n_surf = mag_surf.shape[0]
        n_boundary = mag_boundary.shape[0]

        ang_dist_abs = np.abs(np.tile(ang_surf, (n_boundary,1)).T - np.tile(ang_boundary, (n_surf,1)))

        ind_close = np.logical_or(np.sum(ang_dist_abs<angle_margin, axis=0),
                                  np.sum(np.logical_and((2*pi-angle_margin)<ang_dist_abs, ang_dist_abs<(2*pi+angle_margin)), axis=0) )
                            

        self.surface_points = np.vstack((self.surface_points, boundary_values[~ind_close, :]))
    ...
    def get_gamma(self, position, in_global_frame=False, gamma_scaling=1.0, gamma_type='proportional', ):
        if in_global_frame:
            position = self.transform_global2relative(position)
...
class GaussianEllipseObstacle(ObstacleFromData):
    ...
    def get_normal_direction(self, position):
        ...
        normal_vector =  get_directional_weighted_sum(
            reference_direction=reference_dir, direcptions=normal_directions, weights=weights)
...
class PolygonWithLearnedSurface(Polygon):
    ...
    def get_normal_direction(self, position, in_global_frame=False, normalize=True, normal_calulation_type="distance"):
        ...
        normal_vector_poly =  super().get_normal_direction(position, in_global_frame=False, normalize=True, normal_calulation_type="distance")
        ...
        if len(self.special_surfaces): #>0
            ...
            weights = self.get_distance_weight(np.hstack((gamma_poly, gamma_surfaces)), distance_min=1)
            normal_vector = get_directional_weighted_sum(reference_direction=position, directions=directions, weights=weights)
        ...
        if False: # DEBUGGING
            ...
            plt.quiver(pos_abs[0], pos_abs[1], norm_abs[0], norm_abs[1], color='g', label='normal')
            ...
            for ii in range(pseudo_normal_surfaces.shape[1]):
                pseudo_norm_trans = self.transform_relative2global_dir(pseudo_normal_surfaces[:,ii])
    ...
    def get_angles_to_specialSurfaces(self, position, in_global_frame=False):
        ...
        for ii in range(len(self.special_surfaces)):
            ...
            delta_phi_low = angle_difference_directional(self.special_surfaces_angles[0, ii], angle2pos)
            delta_phi_high = angle_difference_directional(angle2pos, self.special_surfaces_angles[1, ii])
    ...
    def get_gammas_specialSurface(self, position, in_global_frame=False, angle_limit=pi/8):
        if in_global_frame:
            position = self.transform_global2relative(position)
    ...
    def get_pseudoNormals_specialSurfaces(self, position, angle_limit=pi/8):
        ...
        reference_direction = -self.get_reference_direction(position, in_global_frame=False)

        pseudo_normals = np.zeros((self.dim, len(self.special_surfaces)))
        for ii in range(len(self.special_surfaces)):
            if (weights[ii]>0):
                normal_direction = self.special_surfaces[ii].get_normal_direction(position)
                pseudo_normals[:, ii] = weights[ii]*normal_direction + (1-weights[ii])*reference_direction
            else:
    ...
    def find_boundary_surfaces(self, obs_list):
        ...
        for ii in range(len(obs_list)):
            ...
            for pp in range(obs_list[ii].surface_points.shape[0]):
                ...
                if super().get_gamma(obs_list[ii].surface_points[pp, :], in_global_frame=True)<1.5:
    ...
    def intersection_with_special_surfaces(self):
        ...
        for surface_reference in self.special_surfaces:
            ...
            mag, surf_mean_angle = transform_cartesian2polar(mean_surf_point, self.center_position)
    ...
    #     return self.x_obs.T
                ...
                while abs(it_count*d_ang) <= pi:
                    ...
                    position = transform_polar2cartesian(angle=angle, magnitude=radius_surface, center_position=surface_reference.center_position)
