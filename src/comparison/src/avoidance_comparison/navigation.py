...
def get_beta_circle(obstacle, position):
    ...
    if obstacle.is_boundary:
        return (
            obstacle.radius ** 2 - LA.norm(position - obstacle.position) ** 2
        )
    else:
        return (
            LA.norm(position - obstacle.position) ** 2 - obstacle.radius ** 2
        )
...
def get_starset_deforming_factor(
    obstacle, beta, position=None, rel_obs_position=None
):
...
class SphereToStarTransformer(BaseContainer):
    ...
    def __init__(
        self, lambda_constant=1000, attractor_position=None, *args, **kwargs
    ):
    ...
    def get_analytic_switches(
        self, rel_pos_attractor, beta_values, goal_norm_ord=1
    ):
        ...
        if np.sum(ind_zero) == 0:
            beta_bar = (
                np.tile(np.prod(beta_values), (beta_values.shape[0]))
                / beta_values
            )
        ...
        switch_value = scaled_dist / (
            scaled_dist + self.lambda_constant * beta_values
        )
        # breakpoint()
        return switch_value
    ...
    def transform_to_sphereworld(self, position):
        ...
        position_starshape = np.zeros(
            (position.shape[0], beta_values.shape[0])
        )
...
class NavigationContainer(SphereToStarTransformer):
    ...
    def get_epsilon_factor(self):
        ...
        for ii in range(self.n_obstacles):
            beta_i_max[ii] = (
                rho0 + LA.norm(self.center_position)
            ) ** 2 - self[ii].radius ** 2
