...
def widget_ellipses_vectorfield(
    x1=2, x2=2, a1=1, a2=1, p1=1, p2=1, orientation=0, sf=1,
    ...
    vel = [0, 0]
    obs.append(Ellipse(axes_length=axes_length, curvature=curvature,
                       center_position=center_position, orientation=orientation, sf=sf, xd=vel))
    ...
    fig, ax = Simulation_vectorFields(
        xlim, ylim, point_grid=10, obs=obs, pos_attractor=pos_attractor,
        figName='linearSystem_avoidanceCircle', noTicks=False, figureSize=(13.,10),
...
def widgetFunction_referencePoint(
    x1=2, x2=2, orientation=0, refPoint_dir=0, refPoint_rat=0,
    ...
    obs.append(
        Ellipse(axes_length=axes_length, curvature=curvature,
                center_position=center_position, orientation=orientation,
                sf=sf, xd=vel))
...
class WidgetClass_intersection():
    def __init__(self,  x_lim, y_lim, pos_attractor=[12,0]):
        ...
        self.obs.append(
            Ellipse(axes_length=[3,1], curvature=[1,1],
                    center_position=[-14,4], orientation=45/180*pi, sf=2.0))
        self.obs.append(
            Ellipse(axes_length=[3,1], curvature=[1,1],
                    center_position=[-3, 10], orientation=0/180*pi, sf=1.3))
        self.obs.append(
            Ellipse(axes_length=[2.4, 2.4], curvature=[4,4],
                    center_position=[-6, 4], orientation=-80/180*pi, sf=1.1))
        self.obs.append(
            Ellipse(axes_length=[3, 2], curvature=[1,2],
                    center_position=[10, 14], orientation=-110/180*pi, sf=1.5))
...
def run_obstacle_description():
    ...
    x1_widget = FloatSlider(description='Position \( x_1\)',
                            min=x_lim[0], max=x_lim[1], step=0.1, value=6)
    x2_widget = FloatSlider(description='Position \( x_2\)',
                            min=y_lim[0], max=y_lim[1], step=0.1, value=8)

    axis_widget1 = FloatSlider(description='Axis length 1',
                               min=0.1, max=8, step=0.1, value=5)
    axis_widget2 = FloatSlider(description='Axis length 2',
    ...
    margin_widget = FloatSlider(description='Safety Margin',
                                min=1, max=3, step=0.1, value=1.2)

    angle_widget = FloatSlider(description='Orientation',
                               min=-180, max=180, step=1, value=30)

    pointX_widget = FloatSlider(
        description='Point position x', min=x_lim[0], max=x_lim[1],
        step=0.1, value=-3)
    pointY_widget = FloatSlider(
        description='Point position y', min=y_lim[0], max=y_lim[1],
        step=0.1, value=15)
...
def choose_obstacles_number(n_obstacles, WidgetClass, x_lim, y_lim):
    ...
    center1_widget1 = FloatSlider(
        ...
        min=x_lim[0], max=x_lim[1], step=0.1, value=-14, style=style)
    center2_widget1 = FloatSlider(
        description='Center Position \( x_2\)',
        min=y_lim[0], max=y_lim[1], step=0.1, value=4, style=style)
    angle_widget1 = FloatSlider(
        description='Orientation \( \Theta \)',
        min=-180, max=180, step=1, value=45, style=style)

    interact(WidgetClass.set_obstacle_values, it_obs=it_obs, center_position_1=center1_widget1, center_position_2=center2_widget1, orientation=angle_widget1)
...
def example_dynamic_modulation():
    ...
    vel = [0, 0]
    obs.append(Ellipse(
        axes_length=axes_length, curvature=curvature,
        center_position=center_position, orientation=orientation, sf=1))
