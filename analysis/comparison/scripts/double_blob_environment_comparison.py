...
def plot_vectorfield(ax, dynamic_avoider,
                     x_lim, y_lim, n_grid=100, streamplotcolor='#808080'):
    ...
    ax.streamplot(
        ...
        velocities[1, :].reshape(n_grid, n_grid), color=streamplotcolor,
        zorder=-2)
...
def plot_trajectories(
    ax, start_points, dynamic_avoider,
    dt_step=0.01, it_max=1000,
    linecolor='#008000ff', linewidht=4,
    markersize=12,
    plot_attractor=True, plot_start=True,
    ...
    for point in start_points:
        ...
        for ii in range(it_max):
            ...
            position_list[:, ii+1] = velocity * dt_step + position_list[:, ii]

        plt.plot(position_list[0, :], position_list[1, :],
                 color=linecolor, linewidth=linewidht)
        if plot_start:
            plt.plot(position_list[0, 0], position_list[1, 0],
                     's', markersize=markersize, color=markercolor)
        
    if plot_attractor:
        plt.plot(dynamic_avoider.initial_dynamics.attractor_position[0],
                 dynamic_avoider.initial_dynamics.attractor_position[1],
                 'o', color=markercolor, markersize=markersize)
...
def plot_single_blob(save_figure=False):
    ...
    initial_dynamics = LinearSystem(attractor_position=np.array([0.0, 0]),
    ...
    obstacle_environment[0].plot_obstacle(
        ax=ax, fill_color='#ff9955', outline_color=None, plot_center_position=False)
    ...
    plot_trajectories(
        ax=ax, dynamic_avoider=dynamic_avoider, start_points=start_points,
        it_max=1000)

    plot_vectorfield(ax=ax, dynamic_avoider=dynamic_avoider,
...
def plot_double(save_figure=False):
    ...
    for obs in obstacle_environment:
        obs.plot_obstacle(
            ax=ax, fill_color='#ff9955',
            outline_color=None, plot_center_position=False)
...
def plot_circular_control_lyapunov(save_figure=False):
    ...
    for obs in obstacle_environment:
        obs.plot_obstacle(
            ax=ax, fill_color='#00ff00ff', outline_color=None,
    ...
    plot_trajectories(
        ax=ax, dynamic_avoider=dynamic_avoider, start_points=start_points,
        linecolor='black', linewidht=2,
        # startshape='o', markersize=8, markercolor='blue',
        plot_attractor=False, plot_start=False,
        it_max=1000,
        )
    ...
    plt.scatter(start_points[0, :], start_points[1, :], s=70,
