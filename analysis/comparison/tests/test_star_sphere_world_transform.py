...
def plot_star_and_sphere_world(save_figure=False, lambda_constant=None):
    ...
    for point in point_list:
        point_displaced = obstacle_container.transform_to_sphereworld(np.array(point))
    
        plt.plot(point[0], point[1], 'k.')
        plt.plot(point_displaced[0], point_displaced[1], 'b.')

        dir_arrow = np.array([point_displaced[0]-point[0], point_displaced[1]-point[1]])
