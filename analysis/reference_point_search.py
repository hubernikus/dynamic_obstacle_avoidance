...
def main(save_figure=False, n_points=100):
    ...
    if 1 in case :
        ...
        obs1 = Ellipse(axes_length=[2.0, 2.0], orientation=50./180*pi, center_position=[0.0, 0])
        ...
        for ii in range(n_points):
            ...
            normal_vectors_1[:, ii] = obs1.get_normal_direction(position=surface_points_1[:, ii], in_global_frame=True)
        ...
        plt.plot(surface_points_1[0, ii_min], surface_points_1[1, ii_min], '*r', label='Minimum')
    ...
    fig = plt.figure()
    ax = fig.gca(projection='3d')
    surf = ax.plot_surface(angle_position[0, :, :], angle_position[1, :, :], dist_proj,
                           cmap=cm.coolwarm,
                           # linewidth=0,
                           antialiased=False)
    ...
    fig = plt.figure()
    ax = fig.gca(projection='3d')
    surf = ax.plot_surface(angle_position[0, :, :], angle_position[1, :, :], dist_norm,
                           cmap=cm.coolwarm,
                           # linewidth=0,
                           antialiased=False)
