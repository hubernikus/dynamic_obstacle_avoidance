...
def main(save_figure=False, n_grid=50, x_lim=[-2, 4], y_lim=[-3, 3]):
    ...
    surf = axs[0].contourf(x_vals[0, :], y_vals[:, 0],
                           gammas0.reshape(n_grid, n_grid),
                           level_boundaries,
                           extend='max', alpha=alpha_value)
    ...
    surf = axs[1].contourf(x_vals[0, :], y_vals[:, 0],
    ...
    gammas_mixed[ind_inside] = (gamma_margin/(gamma_margin-gammas0[ind_inside])
    ...
    surf = axs[2].contourf(x_vals[0, :], y_vals[:, 0],
    ...
    for ax in axs:
        ...
        for obs in environment:
            ...
            ax.plot(obs.boundary_points_global[0, :],
                    obs.boundary_points_global[1, :], 'k:')
