...
obs1 = Cuboid(axes_length=[0.7, 1.0], orientation=10./180*pi, center_position=[0.0, 0.0], margin_absolut=0.0)
# obs1 = Ellipse(axes_length=[3.0, 0.5], orientation=00./180*pi, center_position=[0.0, 0.0])
obs2 = Ellipse(axes_length=[0.7, 1.0], orientation=30./180*pi, center_position=[2.0, 0.0], margin_absolut=0.2)
...
if 1 in case:
    ...
    for ii in range(n_points):
        ...
        surface_derivative1[:, :, ii] = obs1.get_surface_derivative_angle_num(np.array([phi_1[ii]]), NullMatrix=NullMatrix_1, in_global_frame=True)
        norm_derivative1[:, :, ii] = obs[0].get_normal_derivative_angle_num(np.array([phi_1[ii]]), NullMatrix=NullMatrix_1, in_global_frame=True)
        norm_dist_1[:, ii] = obs[0].get_normal_direction(surface_points_1[:, ii], in_global_frame=True)

        directions_1 = get_angle_space_inverse(np.array([phi_1[ii]]), NullMatrix=NullMatrix_1)
        surface_points_1[:, ii] = obs1.get_local_radius_point(direction=directions_1, in_global_frame=True)
    ...
    for jj in range(n_points):
        ...
        surface_derivative2[:, :, jj] = obs2.get_surface_derivative_angle_num(np.array([phi_2[jj]]), NullMatrix=NullMatrix_2, in_global_frame=True)
        norm_derivative2[:, :,  jj] = obs[1].get_normal_derivative_angle_num(np.array([phi_2[jj]]), NullMatrix=NullMatrix_1, in_global_frame=True)
        
        norm_dist_2[:, jj] = obs[1].get_normal_direction(surface_points_2[:, jj], in_global_frame=True)
        directions_2 = get_angle_space_inverse(np.array([phi_2[jj]]), NullMatrix=NullMatrix_2)
        surface_points_2[:, jj] = obs2.get_local_radius_point(direction=directions_2, in_global_frame=True)
    ...
    for ii in range(n_points):
        for jj in range(n_points):
            ...
            # TODO: check sign
            d_dphi[0, ii, jj] = 1.0/distance[ii, jj]*surface_derivative1[:, :, ii].dot(dist_dir)
            ...
            if dist_type==0:
                ...
                distance[ii, jj] = 0.5*(norm_dist_1[:, ii]-norm_dist_2[:, jj]).T.dot(dist_dir)
    ...
    if show_normals:
        ...
        # Tangents
        plt.quiver(surface_points_1[0, :], surface_points_1[1, :], surface_derivative1[0, :], surface_derivative1[1, :])
        plt.quiver(surface_points_2[0, :], surface_points_2[1, :], surface_derivative2[0, :], surface_derivative2[1,:])
    ...
    if show_stream_plot:
        ...
        plt.streamplot(phi_1, phi_2, d_dphi[0, :,  :].T, d_dphi[1, :,  :].T, zorder=-2, color='k')
...
if 2 in case:
    ...
    while True:
        ...
        surface_derivative1 = obs1.get_surface_derivative_angle_num(np.array([ang_1]), NullMatrix=NullMatrix_1, in_global_frame=True)
        surface_derivative2 = obs2.get_surface_derivative_angle_num(np.array([ang_2]), NullMatrix=NullMatrix_2, in_global_frame=True)

        directions_1 = get_angle_space_inverse(np.array([ang_1]), NullMatrix=NullMatrix_1)
        directions_2 = get_angle_space_inverse(np.array([ang_2]), NullMatrix=NullMatrix_2)

        # directions_1 = np.array([np.cos(ang_1), np.sin(ang_1)])
        # directions_2 = np.array([np.cos(ang_2), np.sin(ang_2)])
        
        surface_point_1 = obs1.get_local_radius_point(direction=directions_1, in_global_frame=True)
        surface_point_2 = obs2.get_local_radius_point(direction=directions_2, in_global_frame=True)

        dist_dir = (surface_point_2 - surface_point_1)
        distance_opt = np.append(distance_opt, [np.linalg.norm(dist_dir)] )

        d_dphi_opt = 0.5/distance_opt[-1]*np.array([-dist_dir.dot(surface_derivative1.squeeze()),
...
if 1 in case and 2 in case:
    ...
    # plt.figure()
    angles_optimization[0, :] = angles_optimization[0, :]
    plt.plot(angles_optimization[0, :], angles_optimization[1, :], 'g', zorder=2, linewidth=4)
