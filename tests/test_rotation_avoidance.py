...
def test_single_circle():
    if True:
        warnings.warn("Rotation avoidance temporarily disabled")
        return
    ...
    assert np.isclose(np.dot(modualted_velocity, (-1) * normal_dir), 0)
    ...
    assert np.allclose(initial_velocity, modualted_velocity)
...
def test_double_ellipse():
    ...
    assert np.isclose(np.dot(modualted_velocity, normal_dir), 0)
