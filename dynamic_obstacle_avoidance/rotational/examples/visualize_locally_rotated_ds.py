...
def draw_initial_locally_rotated(save_figure=False):
    ...
    ds_list.append(
        LocallyRotated(mean_rotation=[-5 / 4 * np.pi]).from_ellipse(obs_list[-1])
    )
    ...
    ds_list.append(
        LocallyRotated(mean_rotation=[-np.pi / 2]).from_ellipse(obs_list[-1])
    )
    ...
    ds_list.append(
        LocallyRotated(mean_rotation=[-np.pi / 4]).from_ellipse(obs_list[-1])
    )
    ...
    ds_list.append(
        LocallyRotated(mean_rotation=[-np.pi / 5]).from_ellipse(obs_list[-1])
    )
    ...
    ds_list.append(LocallyRotated(mean_rotation=[0]).from_ellipse(obs_list[-1]))
    ...
    for ii in range(len(ds_list)):
        ...
        ax.plot(
            ds_list[ii].center_position[0],
            ds_list[ii].center_position[1],
            "k*",
            linewidth=18.0,
            markersize=18,
            zorder=5,
