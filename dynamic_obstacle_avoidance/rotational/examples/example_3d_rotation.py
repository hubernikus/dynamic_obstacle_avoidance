...
from dynamic_obstacle_avoidance.visualization.plot_3d_trajectory import (
...
def evaluate_single_position(
    position, InitialDynamics, ObstacleContainer, func_obstacle_avoidance
):

    initial_velocity = InitialDynamics.evaluate(position)
    modulated_velocity = func_obstacle_avoidance(
        position, initial_velocity, ObstacleContainer
    )
...
def example_single_ellipse_3d():
    ...
    test_poses.append(
        np.array([0.9453695848565005, 0.2816314518962237, 0.4098719224093203])
    )
    test_poses.append(
        np.array([0.9411042172145309, 0.2789087614555541, 0.41201641691364266])
    )
    if False:
        # for test_pos in test_poses:
        # np.set_printoptions(precision=8)
        evaluate_single_position(
            test_pos,
