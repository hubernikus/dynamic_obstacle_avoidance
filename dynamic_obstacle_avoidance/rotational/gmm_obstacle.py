...
class GmmObstacle:
    ...
    def get_one_level_hirarchy(self):
        ...
        for ii in range(self.n_gmms):
            ...
            self.gmm_graph.add_element_with_parent(child=ii, parent=ind_parent)
    ...
    def transform_to_analytic_ellipses(self):
        ...
        for n in range(len(self._gmm.covariances_)):
            if self._gmm.covariance_type == "full":
                ...
                covariances = (
                    np.eye(self._gmm.means_.shape[1]) * self._gmm.covariances_[n]
                )
    ...
    def get_subobstacle_weights(self, position, gamma_min_factor=0.9):
