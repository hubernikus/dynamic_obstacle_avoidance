...
class DynamicEllipse(Ellipse):
    ...
    def __init__(self, x_range, y_range, *args, **kwargs):
        ...
        axes_length = axes_length*(self.axis_range[1] - self.axis_range[0]) + self.axis_range[0]
    ...
    def update_step(self, time_step=0.1, acceleration_rand=0.1, expansion_acc_rand=0.1):
        ...
        for ii in range(self.dim):
            ...
            self.axes_length[ii] = self.axes_length[ii] + time_step*self.expansion_vel[ii]
...
class ObstacleAvoidanceAgent():
    ...
    def update_step(self, obstacle_list=[], delta_time=0.1, initial_velocity=None, check_convergence=True, time=None, vel_min_margin=0.01):
        ...
        self.MetricTracker.update_list(position=self.position, velocity=self.velocity, time=time)
...
def compare_algorithms_random(max_it=1000, delta_time=0.01, max_num_obstacles=5, dim=2, visualize_scene=True, random_seed=None, fig_and_ax_handle=None, fig_num=None, plot_last_image=False, show_legend=True):
    ...
    edge_points = np.array([[obs_xaxis[0], obs_xaxis[0], obs_xaxis[1], obs_xaxis[1]],
    ...
    while True:
        ...
        start_position[0] = x_starting[0] + (x_starting[1]-x_starting[0])*start_position[0]
        start_position[1] = y_starting[0] + (y_starting[1]-y_starting[0])*start_position[1]
        ...
        for agent in agents:
            ...
            if visualize_scene:
                agent.line_handle, = plt.plot(
                    agent.position_list[0, :], agent.position_list[1, :],
                    '--', label=agent.name)
...
def multiple_random_runs(num_runs=2):
    ...
    list_agent_metrics = []
    outcome_keys = ['total', 'has_converged', 'has_collided', 'is_in_local_minima', 'not_converged']
    outcome_dict = {}
    for key in outcome_keys:
        outcome_dict[key] = 0

        
    list_simulation_outcome = [copy.deepcopy(outcome_dict) for ii in range(num_agent_type)]
    ...
    while len(list_initial_distance) == 0 or ii < num_runs:
        ii += 1
        agent_metrics, initial_distance = compare_algorithms_random(visualize_scene=False)
...
def evaluation_metrics(metrics):
    ...
    for aa, agent_dict in zip(np.arange(len(list_agent_metrics[0])), list_agent_metrics[0]) :
        # import pdb; pdb.set_trace()
        for key in agent_dict.keys():
            ...
            if isinstance(agent_dict[key], dict):
                ...
                data = [list_agent_metrics[it][aa][key] for it in range(len(list_agent_metrics))]
    ...
    table_order_list = ['name', 'distance', 'duration', 'linear_velocity_mean', 'linear_velocity_std']
    ...
    # breakpoint()
    return table_str, table_success_str, {'num_success_metrics': num_success_metrics, 'num_total': num_total}
...
if (__name__)=="__main__":
    ...
    if True:
        # comparison_suplots(rand_seed_0=9, save_figure=False)
        comparison_suplots(rand_seed_0=56, save_figure=True)
        # comparison_suplots(rand_seed_0=56, save_figure=False)
