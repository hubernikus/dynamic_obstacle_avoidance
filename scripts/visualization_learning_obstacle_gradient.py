...
def create_normal_vector_field(Obstacles, resolution=20, x_range=[0, 1], y_range=[0, 1]):
...
def create_modulated_vector_field(Obstacles, pos_attractor=None, resolution=80, x_range=[0, 1], y_range=[0, 1], plot_type_quiver=True, max_vel=0.1):
    ...
    for ix in range(res_x):
        for iy in range(res_y):
            ...
            ds_initial[:,ix,iy] = linear_ds_max_vel(pos, attractor=pos_attractor, max_vel=max_vel)
            
            ds_modulated[:,ix,iy] = obs_avoidance_interpolation_moving(pos, ds_initial[:,ix,iy], Obstacles, evaluate_in_global_frame=True, zero_vel_inside=True, velocicity_max=max_vel)
    ...
    if plot_type_quiver:
        ...
        for oo in range(len(Obstacles)):
            Obstacles[oo].draw_obstacle(show_contour=False, fig=fig, ax=ax)
        qv1 = ax.quiver(XX.flatten(), YY.flatten(), ds_initial[0, :, :].flatten(), ds_initial[1, :, :].flatten(), color='#AE204F')
        ...
        qv2 = ax.quiver(XX.flatten(), YY.flatten(), ds_modulated[0, :, :].flatten(), ds_modulated[1, :, :].flatten(), color='#0C537C')
