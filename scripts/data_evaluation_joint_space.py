...
def get_boundary_values(file_in_path, file_in_name, plot_raw_data=False, save_data=True):
    file_type = ".txt"
    ...
    if save_data:
        file_out_path = "/home/lukas/Code/ObstacleAvoidance/dynamic_obstacle_avoidance/data/"
...
def visualize_vectorfield(sensor_data, show_plot=True):
    ...
    ObstacleBoundary = PolygonWithLearnedSurface(edge_points=edge_points , center_position=[0.5, 0.5], is_boundary=True)
    ...
    if True:
        ...
        for ii in range(len(ObstacleBoundary.x_obs_special)):
            plt.plot(ObstacleBoundary.x_obs_special[ii][:,0], ObstacleBoundary.x_obs_special[ii][:,1], 'k-.')
        plt.xlim([-0.02, 1.02])
        plt.ylim([-0.02, 1.02])
        plt.axis('equal')
    ...
    n_grid_x = 50
    n_grid_y = n_grid_x
    YY, XX = np.mgrid[-0.1:1.1:n_grid_x*1j,
                      -0.1:1.1:n_grid_y*1j]
    ...
    for ix in range(n_grid_x):
        for iy in range(n_grid_y):
            ...
            ds_mod[:, ix, iy] = obs_avoidance_interpolation_moving(position, ds_init[:,ix,iy], obs_list)
    ...
    # for ii in [2]:
        ...
        plt.plot(obs_list[ii].center_position[0], obs_list[ii].center_position[1], 'k+', markersize=12, markeredgewidth=4)
...
if get_from_raw_data:
    boundary_values = get_boundary_values(data_set_path, file_name_in, plot_raw_data=True)
else:
    boundary_values = np.load(
        "/home/lukas/Code/ObstacleAvoidance/dynamic_obstacle_avoidance/data/"+file_name_in+".npy")
