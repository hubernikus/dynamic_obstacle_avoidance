...
from dynamic_obstacle_avoidance.dynamical_system.dynamical_system_representation import get_linear_ds, make_velocity_constant
from dynamic_obstacle_avoidance.obstacle_avoidance.gradient_container import GradientContainer
from dynamic_obstacle_avoidance.obstacle_avoidance.human_ellipse import TrackedPedestrian
...
from dynamic_obstacle_avoidance.obstacle_avoidance.angle_math import angle_difference_directional, transform_polar2cartesian
from dynamic_obstacle_avoidance.obstacle_avoidance.crowd_learning_container import CrowdLearningContainer, CrowdCircleContainer
from dynamic_obstacle_avoidance.obstacle_avoidance.ellipse_obstacles import CircularObstacle

from dynamic_obstacle_avoidance.visualization.vector_field_visualization import Simulation_vectorFields, plot_obstacles, plt_speed_line_and_qolo
...
class QoloAnimator(): 
    def __init__(self, x_lim=None, y_lim=None,
        ...
        if obstacle_list is None:
            ...
            self.obstacle_list = CrowdCircleContainer(robot_margin=ROBOT_MARGIN, obs_list=None)
    ...
    def run_circular_world(self, rotate_image=True, attractor_position=np.array([5, 0.1]) ):
        ...
        self.is_pause = False
        fig.canvas.mpl_connect('button_press_event', self.on_click)  # Button Click Enabled
        ...
        self.agent = Obstacle(name="QOLO",
                              ...
                              orientation=0) # Define obstaclen
        ...
        while True:
            ...
            if show_vector_field:
                ...
                line = plt_speed_line_and_qolo(
                    ...
                    obs=self.obstacle_list, fig_and_ax_handle=(fig, ax), dt=0.02)
                ...
                Simulation_vectorFields(
                    ...
                    draw_vectorField=True, show_streamplot=False,
                    point_grid=30,
                    normalize_vectors=False,
                    reference_point_number=True, drawVelArrow=True,
            ...
            if plt_qolo: 
                if rotate_image:
                    ...
                    ax.imshow(arr_img_rotated, extent=[self.agent.position[0]-lenght_x_rotated/2.0, self.agent.position[0]+lenght_x_rotated/2.0,
                                                       self.agent.position[1]-lenght_y_rotated/2.0, self.agent.position[1]+lenght_y_rotated/2.0])

                else:
                    ax.imshow(arr_img, extent=[self.agent.position[0]-length_x/2.0, self.agent.position[0]+length_x/2.0,
                                               self.agent.position[1]-length_y/2.0, self.agent.position[1]+length_y/2.0])
            else:
                ax.plot(self.agent.position[0], self.agent.position[1], 'ko', markeredgewidth=4, markersize=13)

            Simulation_vectorFields(
                self.x_lim, self.y_lim,  obs=self.obstacle_list,
                ...
                # automatic_reference_point=True,
                point_grid=10,
                showLabel=False, fig_and_ax_handle=(fig, ax),
                gamma_distance=RADIUS_HUMAN_PLUS_MARGIN,
                )
            ...
            #)
            ds_inititial = linear_ds_max_vel(self.agent.position,
                                             attractor=attractor_position, vel_max=MAX_SPEED)
            
            ds_modulated = obs_avoidance_interpolation_moving(
                self.agent.position, ds_inititial, self.obstacle_list, 
            ...
            # Update position
            self.agent.position = (self.agent.position
                                   + self.delta_t*self.agent.linear_velocity)
            # import pdb; pdb.set_trace()
            self.agent.orientation = np.arctan2(self.agent.linear_velocity[1], self.agent.linear_velocity[0])
            ...
            plt.quiver(self.agent.position[0], self.agent.position[1], 
                      ds_modulated[0], ds_modulated[1], color='b', alpha=0.7, label="Post-processing DS")
            ...
            try:
                ...
                plt.scatter(self.laser_scan_imitation[0, :],
                            self.laser_scan_imitation[1, :], color='k')
            except:
                pass
                # pass
                # continue
            ...
            if annotate_time:
                ax.annotate(
                    ...
                    xy=[x_lim[1]-(x_lim[1]-x_lim[0])*0.2, y_lim[1]-(y_lim[1]-y_lim[0])*0.08],
                    textcoords='data',
                    size=16, weight="bold")
            ...
            if SAFE_FIG:
                ...
                plt.savefig('dynamic_obstacle_avoidance/figures/' + fig_name + '.png', bbox_inches='tight')
        ...
        if SAFE_ANIMATION:
            ...
            images[0].save('animation_moving'+date_time+".mp4",
                           save_all=True, append_images=images[0:],
                           duration=(time_list[-1]-time_list[0])*100, loop=0)
    ...
    def update_velocity(self, msg_cmd):
        self.agent.linear_velocity= msg_cmd.linear.x * np.array([np.cos(self.agent.orientation), np.sin(self.agent.orientation)])
    ...
    def get_cartesian_from_laserscan(self, msg, delta_angle=0):
        ...
        laser_cartesian = laser_cartesian + np.tile(self.agent.position, (n_points, 1)).T
    ...
    def run_initial_crowd(self, human_radius=HUMAN_RADIUS, robot_margin=ROBOT_MARGIN):
        ...
        for topic, msg, t in self.bag.read_messages('/crowdbot'):
            ...
            for ii in range(len(crowd_list)):
                ...
                if (pos_crowd[0]-human_radius > self.x_lim[0] and
                    pos_crowd[0]+human_radius < self.x_lim[1] and
                    pos_crowd[1]-human_radius > self.y_lim[0] and
                    pos_crowd[1]+human_radius < self.y_lim[1]):

                    vel_crowd = [crowd_list[ii].velocity.linear.x,
                                crowd_list[ii].velocity.linear.z]
                    
                    LocalCrowd.append(CircularObstacle(
                        center_position=pos_crowd, orientation=0,
                        linear_velocity=vel_crowd, angular_velocity=0, 
                        radius=human_radius, margin_absolut=LocalCrowd.robot_margin))

                    LocalCrowd[-1].draw_obstacle() # plot contour
                    
            ax.imshow(arr_img, extent=[-length_x/2.0, length_x/2.0,
                                       -length_y/2.0, length_y/2.0])
    ...
    def run_bag(self):
        ...
        for topic, msg, t in self.bag.read_messages(topics['front_lidar']):
            ...
            angles = np.linspace(msg.angle_min+self.orient_front_lidar,
        ...
        points_front = transform_polar2cartesian(magnitude=magnitudes, angle=angles,
                                                 center_point=self.center_front_lidar)
        ...
        for topic, msg, t in self.bag.read_messages(topics['rear_lidar']):
            ...
            # print('msg', msg)
            magnitudes = np.array(msg.ranges)
            angles = np.linspace(msg.angle_min+self.orient_rear_lidar,
        ...
        points_rear = transform_polar2cartesian(magnitude=magnitudes, angle=angles,
                                                center_point=self.center_front_lidar)
        ...
        reduced_points = transform_polar2cartesian(
            self.obs_list[-1].surface_magnitudes, self.obs_list[-1].surface_angles,
            center_position=self.obs_list[-1].center_position)
        plt.plot(reduced_points[0, :], reduced_points[1, :], 'g.')
        plt.plot(self.obs_list[-1].center_position[0],
                 self.obs_list[-1].center_position[1],
                 'k+', linewidth=18, markeredgewidth=4, markersize=13)

        plt.figure()
        plt.plot(self.obs_list[-1].surface_angles, self.obs_list[-1].surface_magnitudes, '.')
...
if (__name__)=="__main__":
    ...
    if True:
        ...
        obstacle_list = get_conference_room_setup(robot_margin=ROBOT_MARGIN,
                                                  get_surrounding_with_puppet=False)

        # obstacle_list = GradientContainer()

    Animator = QoloAnimator(
    ...
    Animator.run_circular_world(attractor_position=attractor_position,)
