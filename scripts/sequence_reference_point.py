...
target_dir = "/home/lukas/Code/ObstacleAvoidance/dynamic_obstacle_avoidance/figures/sequences"
...
class ReferencePointSequence():
        ...
        for im_it in range(n_figs):
            ...
            for ii in range(len(obs)):
                for jj in range(len(obs)):
                    ...
                    plt.plot(point_bref[0], point_bref[1],
                             'g', marker='+', markeredgewidth=4, markersize=12)

            n=0
            for obstacle in self.obs_list:
                obstacle.draw_obstacle(numPoints=50)
                plt.plot(obstacle.x_obs_sf[0, :], obstacle.x_obs_sf[1, :], color='k', linewidth=2)
                
                plt.plot(obstacle.center_position[0], obstacle.center_position[1], 'k',
                         marker='o',  markeredgewidth=4, markersize=6)
                
                plt.plot(obstacle.global_reference_point[0],
                         obstacle.global_reference_point[1],
                         'r', marker='+', markeredgewidth=3, markersize=10)
            ...
            plt.savefig(os.path.join(self.path_directory,
                                     'shot{}.png'.format(str(im_it).zfill(2))), bbox_inches='tight')
    ...
    def create_gif(self, file_name=None, show_gif=True, duration=800):
        ...
        images[0].save(os.path.join(self.path_directory, "animated.gif"),
                       save_all=True, append_images=images[0:], duration=duration, loop=0)
