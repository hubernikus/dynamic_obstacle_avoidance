...
def get_weighted_angular_mean(angle_weights, angle_tangents2init, init_weight=0):
    ...
    if np.sum(ind_exceedMax):
        return np.sum(
            angle_weights[ind_exceedMax] * angle_tangents2init[ind_exceedMax]
        ) / np.sum(ind_exceedMax)
...
if case == 0:
    ...
    obstacles[-1].orientation = 0.0 / 180 * pi
    obstacles[-1].center_position = [1, 1]

    obstacles.append(copy.deepcopy(obstacles[0]))
    obstacles[-1].orientation = 40.0 / 180 * pi
...
# y_range = [-3, 4]
x_inits = np.vstack(
    ([x_range[1] * np.ones(n_points), np.linspace(y_range[0], y_range[1], n_points)])
)
...
for it_point in range(x_inits.shape[1]):
    ...
    for ii in range(n_steps - 1):
        ...
        if all(
            [
                obstacles[oo].properties["passed_obstacle"]
                for oo in range(len(obstacles))
            ]
        ):
        ...
        check_points = np.vstack(
            (
                np.linspace(positions[0, ii], attractor[0], n_checks),
        ...
        for oo in range(len(obstacles)):
            ...
            if (
                ii >= 1
                and angle_tangent2init[ii - 1, oo]
                * obstacles.get_rotation_direction(oo)
                >= 0
            ):
                # obstacle.properties["passed_obstacle"] = True
                for jj in range(check_points.shape[1]):
                    if (
                        obstacles[oo].get_gamma(
                            check_points[:, jj], in_global_frame=True
                        )
                        < 1
                    ):
                        obstacle.properties["passed_obstacle"] = False
        ...
        if ii == 0:
            delta_velInit = 0
        delta_velInit = angle_difference_directional(
            angle_velInit[ii], angle_velInit[ii - 1]
        )
        ...
        for oo in range(len(obstacles)):
            ...
            normal = obstacles[oo].get_normal_direction(
            ...
            mag, ang = transform_cartesian2polar(
                positions[:, ii], obstacles[oo].global_reference_point
            )

            dist2center = np.linalg.norm(
                positions[:, ii] - obstacles[oo].global_reference_point
            )
            local_radius = obstacles[oo].get_radius_of_angle(ang, in_global_frame=True)
            weights[oo] = get_local_weight(
                dist2center, local_radius, r_boundary=r_boundary
            )
            ...
            if weights[oo] > 0:
                if obstacles.is_outside_influence_region(oo):
                    ...
                    if set_vortex_direction:
                        if obstacles.get_rotation_direction(
                            index=oo
                        ):  # already defined
                            pass
                ...
                angle_tangent2init_ref = angle_difference_directional(
                    angle_tangents_list[ii, oo], angle_velInit[ii]
                )
                ...
                if False:
                    plt.quiver(
                        positions[0, ii],
                        positions[1, ii],
                        np.cos(angle_tangent2init_ref + angle_velInit[ii]),
                        np.sin(angle_tangent2init_ref + angle_velInit[ii]),
                        color="r",
                        scale=15,
                    )
                    ...
                    plt.quiver(
                        ...
                        np.sin(angle_tangents_list[ii, oo]),
                        color="g",
                        scale=15,
                    )
                ...
                if obstacles.is_outside_influence_region(oo):
                    ...
                    if index_siblings.shape[0] > 1:
                        ...
                        ind_nonzero = (
                            np.array(angle_tangent2init[ii - 1, index_siblings]) != 0
                        )
                        it_nonzero = ii - 1
                        if not np.sum(ind_nonzero):
                            ind_nonzero = (
                                np.array(angle_tangent2init[ii, index_siblings]) != 0
                            )
                            it_nonzero = ii
                    ...
                    if np.sum(ind_nonzero):
                        ...
                        referenceDirection_knownObstacle = obstacles[
                            first_ind
                        ].get_reference_direction(
                            positions[:, ii], in_global_frame=True
                        )
                        referenceDirection_newObstacle = obstacles[
                            oo
                        ].get_reference_direction(
                            positions[:, ii], in_global_frame=True
                        )
                        ...
                        if (
                            np.cross(
                                referenceDirection_newObstacle,
                                referenceDirection_knownObstacle,
                            )
                            >= 0
                        ):
                            while (
                                angle_tangent2init[ii, oo]
                                > angle_tangent2init[it_nonzero, first_ind]
                            ):
                    ...
                    delta_angle_tangent = angle_difference_directional(
                        angle_tangents_list[ii, oo], angle_tangents_list[ii - 1, oo]
                    )

                    # print('tang init 0', angle_tangent2init[ii-1, oo])
                    # Continuous Integration
                    angle_tangent2init[ii, oo] = (
                        angle_tangent2init[ii - 1, oo]
                        + delta_angle_tangent
                        - delta_velInit
                    )

                    # print('tang init 1', angle_tangent2init[ii, oo])
                    angle_tangent2init[ii, oo] = windup_smoothening(
                        angle_tangent2init[ii, oo], angle_tangent2init_ref
                    )
        ...
        if np.sum(weights):
            ...
            # Angle weight
            delt_angle_desired = get_weighted_angular_mean(
                angle_weights=weights, angle_tangents2init=angle_tangent2init[ii, :]
            )
            ...
            velocities[:, ii] = np.linalg.norm(vel_init[:, ii]) * np.array(
                [np.cos(angle_desired), np.sin(angle_desired)]
            )
        else:
            velocities[:, ii] = vel_init[:, ii]
        positions[:, ii + 1] = positions[:, ii] + velocities[:, ii] * dt
        ...
        if False:
            ...
            for oo in range(len(obstacles)):
                ...
                plt.quiver(
                    ...
                    np.sin(angle_tangent2init[ii, oo] + angle_velInit[ii]),
                    color="b",
                    label="tangent",
                    scale=arrow_sacle,
                )
            ...
            plt.quiver(
                positions[0, ii],
                positions[1, ii],
                np.cos(angle_velInit[ii]),
                np.sin(angle_velInit[ii]),
                color="r",
                label="initial ds",
                scale=arrow_sacle,
            )
            ...
            plt.quiver(
                positions[0, ii],
                positions[1, ii],
                normals_list[0, ii, oo],
                normals_list[1, ii, oo],
                color="m",
                label="normal_real",
                scale=arrow_sacle,
            )
            plt.plot(
                [obstacles[oo].global_reference_point[0], positions[0, ii]],
                [obstacles[oo].global_reference_point[1], positions[1, ii]],
                "k--",
            )
            plt.legend()
    ...
    if it_point == 0:
        for oo in range(len(obstacles)):
            # plt.figure()
            plt.plot(obstacles[oo].x_obs[:, 0], obstacles[oo].x_obs[:, 1], "k")
            obstacle_boundary = obstacles[oo].draw_obstacle(
                a_temp=np.array(obstacles[oo].axes_length) + r_boundary
            )
    ...
    for ii in range(obstacles.num_families):
        ...
        plt.plot(
            obstacles.family_center[0, label],
            obstacles.family_center[1, label],
            "b.",
            markersize="20",
            linewidth="20",
        )
...
if save_figure:
    fig.savefig(
        "../figures/circeling_starshapedFlower_npoints{}_orientation{}_ylim{}to{}.pdf".format(
            n_points, orientation_object, y_range[0], y_range[1]
        )
    )
