...
def visualize_overrotation_two_ellipse(
    save_figure=False,
    comparison_quiver=False,
    n_resolution=20,
):
    dim = 2
    ...
    if comparison_quiver:
        ...
        stream_color = "blue"
        ax.streamplot(
            positions[0, :].reshape(nn, nn),
            positions[1, :].reshape(nn, nn),
            rotated_velocities[0, :].reshape(nn, nn),
            rotated_velocities[1, :].reshape(nn, nn),
            color=stream_color,
            # zorder=3,
        )
