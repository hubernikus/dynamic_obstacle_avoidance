...
def visualization_gamma(z_val=None):
    ...
    plt.plot(obs.boundary_points_local[0, :], obs.boundary_points_local[1, :],
    ...
    cs = plt.contourf(positions[0, :, :], positions[1, :, :],  gammas, 
...
def visualization_angle_weight(z_val=None):
    ...
    print('Inflation parameter (inflation_parameter={})'.format(obs.inflation_percentage))
    ...
    # for it_plane in [3]:
        ...
        cs = plt.contourf(positions[0, :, :], positions[1, :, :],  angle_weights[it_plane, :, :],
    ...
    for it_plane in range(obs.n_planes):
        ...
        for ix in range(n_grid):
            for iy in range(n_grid):
                positions[:, ix, iy] = [x_vals[ix], y_vals[iy], z_val]
                # In local frame only
                normal_vectors[:, ix, iy] = obs._get_normal_direction_numerical_to_plane(
