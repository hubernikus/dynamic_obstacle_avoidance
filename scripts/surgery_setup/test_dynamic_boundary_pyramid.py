...
def test_gamma_proportional():
    ...
    while(ii < 100):
        ...
        if obs.is_boundary: # fac is nonzero
            ...
            assert gamma2>=gamma1, "Gamma not increasing with decreasing distance from origin."
        else:
            assert gamma2<=gamma1, "Gamma not decreasing with increasing distance from origin."

            # Only obstacle (not for wall) 
            assert not (gamma1 ^ np.linalg.norm(pos)), "Zero gamma value outside origin."
