...
def main():
    ...
    ii = 1
    sensor_data = {}
    sensor_data["angle"] = np.arange(len(data[ii]["ranges"]))*data[ii]["increment"]+data[ii]["angle_min"]
    ...
    # plt.figure()
    data_cartesian = ObstacleFromLaser.transform_polar2cartesian(self=None,
    ...
    # for ii in [2]:
        ...
        plt.plot(ObstaclesScanned.obs_list[ii].center_position[0], ObstaclesScanned.obs_list[ii].center_position[1], 'k+', markersize=12, markeredgewidth=4)
    ...
    points_init = np.vstack((x_lim[0]*np.ones(n_points),
                             points_init[1:-1]))
    ...
    if show_quiverPlot:
        ...
        Simulation_vectorFields(x_lim, y_lim, N_resol, obs, xAttractor=xAttractor, saveFigure=saveFigures, figName='linearSystem_avoidanceCube', noTicks=False, figureSize=(6.,5), automatic_reference_point=False)
...
def plot_streamlines_sensory(sensor_data, points_init, ax, attractorPos=[0,0],
    ...
    for iSim in range(max_simu_step):
        ...
        for j in range(n_points):
            ...
            time_clust, time_obstacleLearning = ObstaclesScanned.get_obstacle_from_scan(sensor_data=sensor_data)
            ...
            x_pos[:, iSim+1,j] = obs_avoidance_rk4(
                dt, x_pos[:,iSim, j], obs=ObstaclesScanned.obs_list, x0=attractorPos,
    ...
    print('Average time: {} ms'.format(np.round((time_tot)/(it_count*n_points)*1000),5) )
    print('Modulation calculation total: {} s'.format(np.round(time_tot), 4))

    
    percentage_times = np.round(np.array([time_tot_createFromScan, time_tot_clustering, time_tot_obstacleLearning, time_tot_modulation])/time_tot*100, 1 )
    
    print("Percentage time [time to get obstacles from scan: {}%  --  clustering: {}% -- obstacle learning: {}% -- modulation (RK4): {}% ]".
          format(percentage_times[0], percentage_times[1], percentage_times[2], percentage_times[3]))
