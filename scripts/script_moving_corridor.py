...
def plot_multihulls(
    obstacle_environment, x_lim, y_lim, ax=None, show_margin=True, plot_boundary_multicolor=False):
    ...
    boundary_polygon = plt.Polygon(
        outer_boundary, alpha=0.8, zorder=-4
        )
    ...
    def evaluate_multiboundary(self, position, initial_velocity=None):
        ...
        for oo, obs in enumerate(self._obstacle_list):
            ...
            if initial_velocity is None:
                velocity = self.dynamical_systems[oo].evaluate(
                    position)
            else:
                velocity = initial_velocity
                
            
            mod_vel_oo = obs_avoidance_interpolation_moving(
...
def main_moving_in_corridor(n_resolution=100, save_figure=False):
    ...
    plot_multihulls(obstacle_environment, x_lim, y_lim, ax=ax, plot_boundary_multicolor=True)
    ...
    fig, ax = plt.subplots(figsize=figsize)
    obstacle_environment.dynamical_systems = [obstacle_environment.dynamical_systems[-1]]
...
def plot_weights(n_resolution=100):
    ...
    fig, axs = plt.subplots(1, len(obstacle_environment), figsize=(19, 4.))
