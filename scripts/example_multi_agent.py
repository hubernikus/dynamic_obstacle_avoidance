...
def main(simulationNumber=0, saveFigures=False):
    if simulationNumber==1:
        ...
        for oo in range(n_obs):
            obs.append(Ellipse(axes_length=[rad_robot, rad_robot],
                               center_position=pos_obs[:, oo],
                               orientation=0, margin_absolut=rad_robot))
                           # orientation=-25*pi/180, margin_absolut=0.0 ))

        obs[0].draw_obstacle()

        run_animation_multibody(x_init, obs, x_range=x_range, y_range=y_range,
                                dt=0.020, N_simuMax=800, convergenceMargin=0.3, sleepPeriod=0.001,
                                attractorPos=attr_list, velocity_max=3.0,
                                saveFigure=saveFigures, animationName="multibody_")
