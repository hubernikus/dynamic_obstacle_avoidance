...
def kernel_spherical(XX, YY, sigma=10):
    ...
    kk = 1 - 3./2*dist/sigma + 0.5*(dist/sigma)**2
    return kk
...
if show_cartesian_simulation:
    ...
    n_resol = 30
    xx, yy = np.meshgrid(np.linspace(x_limit[0], x_limit[1], n_resol),
...
if show_radial_simulation:
    ...
    n_resol = 30
    pp, rr = np.meshgrid(np.linspace(-pi, pi, n_resol),
                         np.linspace(0, 2, n_resol))
