...
if (__name__) == "__main__":
    ...
    for ix in range(n_grid):
        for iy in range(n_grid):
            ...
            mag, ang = transform_cartesian2polar(position, obstacles[oo].global_reference_point)

            local_radius = obstacles[oo].get_radius_of_angle(ang, in_global_frame=True)

            pos_init[:, ix, iy] = position
            pos_surface[:, ix, iy] = np.array([np.cos(ang), np.sin(ang)])*local_radius \
            ...
            normal[:, ix, iy] = obstacles[oo].get_normal_direction(position, in_global_frame=True)


    plt.plot(pos_surface[0, :, :], pos_surface[1, :, :], 'g.')
    plt.quiver(pos_surface[0, :, :], pos_surface[1, :, :], normal[0, :, :], normal[1, :, :], color='g')
    ...
    if False:
        cs = ax.contourf(pos_init[0, :, :], pos_init[1, :, :], gamma, cmap=plt.cm.coolwarm, alpha=0.8)
