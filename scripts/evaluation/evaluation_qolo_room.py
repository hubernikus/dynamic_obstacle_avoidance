...
class DynamicSimulator():
    def __init__(self, input_file, attractor_position=None, x_range=[-10,10], y_range=[-10,10], figSize=(8,7)):
        ...
        self.agent = Ellipse(center_position=np.zeros(2),
                             orientation=0,
    ...
    def setup_environment(self):
        ...
        if not line[0]=="time":
            raise ValueError("Unkown file structure with starting value <{}>.".format(line[0]))
    ...
    def main_loop(self):
        ...
        while(True):
            ...
            self.agent_trajectory = np.hstack((self.agent_trajectory,
                                               self.agent.center_position.reshape(self.dim, 1) ))
    ...
    def plot_obstacle_environment(self):
        ...
        for ii in range(len(self.obs)):
            if self.obs[ii].is_boundary:
                ...
                obs_polygon = plt.Polygon(self.obs[ii].boundary_points_global[:, :2],
                                          alpha=1.0, zorder=-1)
            ...
            boundary_line, = plt.plot(self.obs[ii].boundary_points_margin_global[:, 0],
            ...
            reference_point, = self.ax.plot([],[], 'k+', animated=True, linewidth=18, markeredgewidth=4, markersize=13)
        ...
        while(True):
            ...
            # elif line[0]=="object":
                ...
                while(True):
                    ...
                    line = re.split(',', self.simulation_file.readline()[:-1])
                    if (line[0]=="center_position" or line[0]=="linear_velocity"
                ...
                if not reference_point is None:
                    self.obs[-1].set_reference_point(reference_point, in_global_frame=True)
