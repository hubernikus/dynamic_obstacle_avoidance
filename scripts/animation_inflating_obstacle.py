...
def animation_wall_with_door(max_it=1000, delta_time=0.01, max_num_obstacles=5, dim=2, visualize_scene=True, random_seed=None):
    ...
    for ii in range(max_it):
        ...
        # attractor_position = obs_list['RoomWithDoor'].get_global_gap_center()
        attractor_position = obs_list['RoomWithDoor'].get_gap_outside_point(dist_relative=2)
        ...
        if False:
            plt.plot(obs_list['RoomWithDoor'].center_position[0], obs_list['RoomWithDoor'].center_position[1], 'k+')

        for agent in agents:
            initial_velocity = linear_ds_max_vel(
                position=agent.position, attractor=attractor_position, vel_max=0.5,
            )
            ...
            if visualize_scene:
                plt.plot(agent.position_list[0, :], agent.position_list[1, :],
                         '--', label=agent.name, zorder=5, color='blue')
                plt.plot(agent.position_list[0, -1], agent.position_list[1, -1],
                         'o', label=agent.name, zorder=5, color='blue')
