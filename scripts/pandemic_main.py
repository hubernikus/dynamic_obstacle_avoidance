...
class CrowdSimulation():
    ...
    def __init__(self, n_points=10, pos_points=None):
        ...
        if pos_points is None:
            ...
            self.position[0, :] = self.position[0, :]*(ax.get_xlim()[1]-ax.get_xlim()[0]) + ax.get_xlim()[0]
            self.position[1, :] = self.position[1, :]*(ax.get_ylim()[1]-ax.get_ylim()[0]) + ax.get_ylim()[0]
        else:
        ...
        self.point = ax.scatter([], [], color='k', linewidths=1, s=self.marker_width**2)
        # self.position[:, 0] = 0
        ...
        self.sick_cols = np.array([[32, 165, 41],
                                   [138, 165, 32]])/255.
        self.healthy_col = np.array([61, 68, 162])/255.
        self.healed_col = np.array([196, 165, 39])/255.
        self.vaccinated_col = np.array([196, 65, 39])/255.
    ...
    def animate(self, it_anmimation, dt=0.05, ):
        
        # dt proportional to interval?
        dynamic_acc = np.random.normal(0.0, self.sigma_acc, dim*self.n_points).reshape(dim, self.n_points)
        ...
        if any(ind_too_fast):
            self.dynamic_vel[:, ind_too_fast] = self.dynamic_vel[:, ind_too_fast]/velocity_magnitude[ind_too_fast] * self.max_velocity
    ...
    def update_health(self, healing_steps=0.001):
        ...
        if np.sum(still_sick):
            ...
            self.scatter_color[ind_still, :3] =  sick_status* np.tile(self.sick_cols[0, :], (np.sum(still_sick), 1)) + (1 - sick_status) * np.tile(self.sick_cols[1, :], (np.sum(still_sick), 1))

        if np.sum(np.logical_not(still_sick)):
            ind_healed = np.arange(ind_sick.shape[0])[ind_sick][np.logical_not(still_sick)]
            self.is_healed[ind_healed] = True
            self.scatter_color[ind_healed, :3] = np.tile(self.healed_col, (np.sum(ind_healed.shape[0]), 1))
    ...
    def verify_inter_collision(self, min_distance=0.3):
        ...
        ind_uptriangle = (np.triu(np.ones(n_points)) - np.diag(np.ones(n_points)) ).astype(bool).flatten()
        ind_numb_uptriangle = np.arange(ind_uptriangle.shape[0])[ind_uptriangle]

        pos_arr1 = np.swapaxes(np.tile(positions,   (n_points, 1, 1)), 0, 1).reshape(dim, -1)[:, ind_uptriangle]
        pos_arr2 = np.swapaxes(np.tile(positions.T, (n_points, 1, 1)), 0, 2).reshape(dim, -1)[:, ind_uptriangle]
        ...
        for ind1, ind2 in zip(indices1, indices2):
            ...
            for ii, jj in zip([ind1, ind2], [ind2, ind1]):
                if self.sick_status[ii]>0:
                    if self.sick_status[jj] < 0 and not (self.is_healed[jj] or self.is_vaccinated[jj]):
...
if True:
    ...
    anim = animation.FuncAnimation(fig, Simulator.animate, init_func=Simulator.setup, frames=1000, interval=20, blit=True)
